...
public final class LegacyGLRedirects {
    ...
    private static final ThreadLocal<org.joml.Matrix4f> tempMatrixTL =
            ThreadLocal.withInitial(org.joml.Matrix4f::new);

    public static void glFrustum(double left, double right, double bottom, double top, double zNear, double zFar) {
        float l = (float) left, r = (float) right, b = (float) bottom, t = (float) top;
        float n = (float) zNear, f = (float) zFar;
        org.joml.Matrix4f m = tempMatrixTL.get();
    ...
    public static void gluPerspective(float fovy, float aspect, float zNear, float zFar) {
        ...
        org.joml.Matrix4f persp = tempMatrixTL.get();
    ...
    public static void gluLookAt(float eyeX, float eyeY, float eyeZ,
        ...
        org.joml.Matrix4f lookAt = tempMatrixTL.get().identity().lookAt(
    ...
    public static void glEnable(int cap) {
        ...
        switch (cap) {
            ...
            case 0x0BC0 -> st.enableAlphaTest();                                  // GL_ALPHA_TEST
            case 0x0B50 -> st.enableLighting();                                    // GL_LIGHTING
            case 0x0B60 -> st.enableFog();                                         // GL_FOG
            case 0x0DE1 -> st.enableTexture2D(st.getActiveTextureUnit());           // GL_TEXTURE_2D
            case 0x0BA1 -> st.enableNormalize();                                   // GL_NORMALIZE
            case 0x803A -> st.enableRescaleNormal();                               // GL_RESCALE_NORMAL
            case 0x0B57 -> st.enableColorMaterial();                               // GL_COLOR_MATERIAL
            case 0x4000, 0x4001, 0x4002, 0x4003,                                  // GL_LIGHT0-7
                 0x4004, 0x4005, 0x4006, 0x4007 -> st.enableLight(cap - 0x4000);
            case 0x0C60 -> st.enableTexGen(0);                                     // GL_TEXTURE_GEN_S
            case 0x0C61 -> st.enableTexGen(1);                                     // GL_TEXTURE_GEN_T
            case 0x0C62 -> st.enableTexGen(2);                                     // GL_TEXTURE_GEN_R
            case 0x0C63 -> st.enableTexGen(3);                                     // GL_TEXTURE_GEN_Q
            case 0x3000, 0x3001, 0x3002, 0x3003,                                  // GL_CLIP_PLANE0-5
                 0x3004, 0x3005 -> st.enableClipPlane(cap - 0x3000);
            // Tracked core state — dirty-flagged, skips redundant GL
            case 0x0B71 -> st.enableDepthTest(true);                               // GL_DEPTH_TEST
            case 0x0BE2 -> st.enableBlend(true);                                   // GL_BLEND
    ...
    public static void glDisable(int cap) {
        ...
        switch (cap) {
            // Removed state — software-tracked only
            case 0x0BC0 -> st.disableAlphaTest();
            case 0x0B50 -> st.disableLighting();
            case 0x0B60 -> st.disableFog();
            case 0x0DE1 -> st.disableTexture2D(st.getActiveTextureUnit());
            case 0x0BA1 -> st.disableNormalize();
            case 0x803A -> st.disableRescaleNormal();
            case 0x0B57 -> st.disableColorMaterial();
            case 0x4000, 0x4001, 0x4002, 0x4003,
                 0x4004, 0x4005, 0x4006, 0x4007 -> st.disableLight(cap - 0x4000);
            case 0x0C60 -> st.disableTexGen(0);
            case 0x0C61 -> st.disableTexGen(1);
            case 0x0C62 -> st.disableTexGen(2);
            case 0x0C63 -> st.disableTexGen(3);
            case 0x3000, 0x3001, 0x3002, 0x3003,
                 0x3004, 0x3005 -> st.disableClipPlane(cap - 0x3000);
            // Tracked core state — dirty-flagged, skips redundant GL
            case 0x0B71 -> st.enableDepthTest(false);
            case 0x0BE2 -> st.enableBlend(false);
    ...
    // ═══════════════════════════════════════════════════════════════════
    // Core-profile state — route through CoreStateTracker for dirty-flagging
    // (prevents desync when mods call GL11/GL14 directly)
    // ═══════════════════════════════════════════════════════════════════
    ...
    public static void glBlendFuncSeparate(int srcRGB, int dstRGB, int srcAlpha, int dstAlpha) {
        CoreStateTracker.INSTANCE.blendFuncSeparate(srcRGB, dstRGB, srcAlpha, dstAlpha);
    }
    ...
    public static void glFogf(int pname, float param) {
        ...
        switch (pname) {
            case 0x0B62 -> state.setFogDensity(param);   // GL_FOG_DENSITY
            case 0x0B63 -> state.setFogStart(param);      // GL_FOG_START
            case 0x0B64 -> state.setFogEnd(param);         // GL_FOG_END
    ...
    public static void glShaderSource(int shader, CharSequence source) {
        int type = org.lwjgl.opengl.GL20.glGetShaderi(shader, org.lwjgl.opengl.GL20.GL_SHADER_TYPE);
        String converted = ShaderConverter.convert(type, source.toString());
        org.lwjgl.opengl.GL20.glShaderSource(shader, converted);
    }
    ...
    public static void glUseProgram(int program) {
        ...
        ModShaderBridge.onUseProgram(program);
    }

    public static void glDeleteProgram(int program) {
        ModShaderBridge.onDeleteProgram(program);
    ...
    // ── Texture readback — convert deprecated formats ──────────────

    public static void glGetTexImage(int target, int level, int format, int type, java.nio.IntBuffer pixels) {
        org.lwjgl.opengl.GL11.glGetTexImage(target, level, fixDeprecatedFormat(format), type, pixels);
    }
    ...
    public static void glGetFloatv(int pname, FloatBuffer params) {
        ...
        if (queryRemovedFloatState(pname, params)) return;
    ...
    public static void glGetDoublev(int pname, java.nio.DoubleBuffer params) {
        ...
        // Try removed float state
        FloatBuffer fb = org.lwjgl.BufferUtils.createFloatBuffer(4);
        if (queryRemovedFloatState(pname, fb)) {
            for (int i = 0; i < fb.position() && i < params.remaining(); i++) {
                params.put(params.position() + i, fb.get(i));
            }
            return;
        }
    ...
    private static Integer queryRemovedIntegerState(int pname) {
        ...
        return switch (pname) {
            ...
            case 0x0B52 -> st.getShadeModel();                             // GL_SHADE_MODEL
            case 0x0BC0 -> st.isAlphaTestEnabled() ? 1 : 0;               // GL_ALPHA_TEST
            case 0x0BC1 -> st.getAlphaFunc();                              // GL_ALPHA_TEST_FUNC
            case 0x0B65 -> st.getFogMode();                                // GL_FOG_MODE
            case 0x0B50 -> st.isLightingEnabled() ? 1 : 0;                // GL_LIGHTING
            case 0x0B60 -> st.isFogEnabled() ? 1 : 0;                     // GL_FOG
            case 0x0B57 -> st.isColorMaterialEnabled() ? 1 : 0;           // GL_COLOR_MATERIAL
            // Tracked core state
            case 0x0B71 -> st.isDepthTestEnabled() ? 1 : 0;               // GL_DEPTH_TEST
            case 0x0B72 -> st.getDepthFunc();                              // GL_DEPTH_FUNC
            case 0x0B73 -> st.getDepthMask() ? 1 : 0;                     // GL_DEPTH_WRITEMASK
            case 0x0BE2 -> st.isBlendEnabled() ? 1 : 0;                   // GL_BLEND
            case 0x0BE1 -> st.isBlendEnabled() ? 1 : 0;                   // GL_BLEND_SRC (compat alias)
            case 0x0B44 -> st.isCullEnabled() ? 1 : 0;                    // GL_CULL_FACE
            case 0x0B45 -> st.getCullFaceMode();                           // GL_CULL_FACE_MODE
            case 0x8037 -> st.isPolygonOffsetEnabled() ? 1 : 0;           // GL_POLYGON_OFFSET_FILL
            default -> null;
        };
    }

    /**
     * Intercept removed float state queries. Returns true if handled.
     * Writes results starting at params.position().
     */
    private static boolean queryRemovedFloatState(int pname, FloatBuffer params) {
        CoreStateTracker st = CoreStateTracker.INSTANCE;
        int pos = params.position();
        switch (pname) {
            case 0x0B00: // GL_CURRENT_COLOR
                params.put(pos, st.getColorR()); params.put(pos + 1, st.getColorG());
                params.put(pos + 2, st.getColorB()); params.put(pos + 3, st.getColorA());
                return true;
            case 0x0B62: // GL_FOG_DENSITY
                params.put(pos, st.getFogDensity());
                return true;
            case 0x0B63: // GL_FOG_START
                params.put(pos, st.getFogStart());
                return true;
            case 0x0B64: // GL_FOG_END
                params.put(pos, st.getFogEnd());
                return true;
            case 0x0B66: // GL_FOG_COLOR
                params.put(pos, st.getFogR()); params.put(pos + 1, st.getFogG());
                params.put(pos + 2, st.getFogB()); params.put(pos + 3, st.getFogA());
                return true;
            case 0x0BC2: // GL_ALPHA_TEST_REF
                params.put(pos, st.getAlphaRef());
                return true;
            default:
                return false;
        }
    ...
    public static boolean glIsEnabled(int cap) {
        ...
        return switch (cap) {
            // Removed state
            case 0x0BC0 -> st.isAlphaTestEnabled();                        // GL_ALPHA_TEST
            case 0x0B50 -> st.isLightingEnabled();                         // GL_LIGHTING
            case 0x0B60 -> st.isFogEnabled();                              // GL_FOG
            case 0x0DE1 -> st.isTexture2DEnabled(st.getActiveTextureUnit()); // GL_TEXTURE_2D
            case 0x0BA1 -> st.isNormalizeEnabled();                        // GL_NORMALIZE
            case 0x803A -> st.isRescaleNormalEnabled();                    // GL_RESCALE_NORMAL
            case 0x0B57 -> st.isColorMaterialEnabled();                    // GL_COLOR_MATERIAL
            case 0x4000, 0x4001, 0x4002, 0x4003,                          // GL_LIGHT0-7
                 0x4004, 0x4005, 0x4006, 0x4007 -> st.isLightEnabled(cap - 0x4000);
            case 0x0C60 -> st.isTexGenEnabled(0);                          // GL_TEXTURE_GEN_S
            case 0x0C61 -> st.isTexGenEnabled(1);                          // GL_TEXTURE_GEN_T
            case 0x0C62 -> st.isTexGenEnabled(2);                          // GL_TEXTURE_GEN_R
            case 0x0C63 -> st.isTexGenEnabled(3);                          // GL_TEXTURE_GEN_Q
            case 0x3000, 0x3001, 0x3002, 0x3003,                          // GL_CLIP_PLANE0-5
                 0x3004, 0x3005 -> st.isClipPlaneEnabled(cap - 0x3000);
            // Tracked core state
            case 0x0B71 -> st.isDepthTestEnabled();                        // GL_DEPTH_TEST
            case 0x0BE2 -> st.isBlendEnabled();                            // GL_BLEND
            case 0x0B44 -> st.isCullEnabled();                             // GL_CULL_FACE
            case 0x8037 -> st.isPolygonOffsetEnabled();                    // GL_POLYGON_OFFSET_FILL
            // Untracked core state — query GL
            default -> org.lwjgl.opengl.GL11.glIsEnabled(cap);
        };
    }
